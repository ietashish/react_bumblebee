/**
 * @license
 * at.js 2.0.0-2 | (c) Adobe Systems Incorporated | All rights reserved
...
function createHtml(offers) {
  ...
  action[CONTENT] = join("", flatten(map(getHtml, contents)));
...
function hasAuthenticatedState(value) {
  return !isNil(value[AUTHENTICATED_STATE]);
}
...
function getCustomerIdsParams(visitor) {
  return reduce(function (acc, item) {
    var integrationCode = item[INTEGRATION_CODE];
    if (hasId(item)) {
      acc["" + MC_PREFIX + integrationCode + "." + ID] = item[ID];
    }
    if (hasAuthenticatedState(item)) {
      acc["" + MC_PREFIX + integrationCode + "." + AUTH_STATE] = getAuthState(item[AUTHENTICATED_STATE]);
    }
    return acc;
  }, {}, getCustomerIds(visitor));
}
...
function getVisitorCustomerIdsValues() {
...
function getUrl(config, getCluster) {
  ...
  return join("", [scheme, SCHEME_SEPARATOR, getServerDomain(getCluster, clientCode, serverDomain, overrideMboxEdgeServer), getPath(clientCode)]);
}
...
function transform(config, win, http, processRedirect, request, response) {
  ...
  return flow([handleSessionId, handleDeviceId, handleEdgeCluster, handleTrace, handleError, handleDisabled, handleOffers])(response);
}
...
function getBrowserParameters() {
  ...
  result[DEVICE_PIXEL_RATIO] = getPixelRatio();
...
function executeGetOffer(isDeliveryEnabled, validateGetOfferOptions, ajax, requestStart, requestSucceeded, requestFailed, config, options) {
...
function getOffer(options) {
  executeGetOffer(isDeliveryEnabled, validateGetOfferOptions, ajax, notifyRequestStart, notifyRequestSucceeded, notifyRequestFailed, getConfig(), options);
}
...
function isNotClickTracking(action) {
  return !isClickTracking(action);
}
...
function hideActions(actions) {
  ...
  var noClickActions = filter(isNotClickTracking, actions);
  var cssSelectors = filter(isNotBlank, map(getCssSelector, noClickActions));
...
function shouldRender(action) {
  ...
  var type = action[ACTION];
  if (type === CUSTOM_CODE && action[PAGE]) {
    return true;
  }
  var cssClass = "" + ACTION_CSS_PREFIX + id;
  var selector = action[SELECTOR];
...
function applyOffer(options) {
  ...
  executeRender(notifyActionsHidden, handleClick, clonedActions).then(function () {
...
var PROFILE_PARAMETERS = "profileParameters";
var CHANNEL = "channel";
var WINDOW = "window";
var SCREEN = "screen";
var PIXEL_RATIO = "pixelRatio";
var ORIENTATION = "orientation";
var BROWSER = "browser";
var USER_AGENT = "userAgent";
var HOST = "host";
var REFERRING_URL = "referringUrl";
var TIME_OFFSET = "timeOffsetInMinutes";
var CONTEXT = "context";
var QA_MODE = "qaMode";
var TOKEN = "token";
var LISTED_ACTIVITIES_ONLY = "listedActivitiesOnly";
var EVALUATE_AS_TRUE_AUDIENCE_IDS = "evaluateAsTrueAudienceIds";
var EVALUATE_AS_FALSE_AUDIENCE_IDS = "evaluateAsFalseAudienceIds";
var PREVIEW_INDEXES = "previewIndexes";
var ACTIVITY_INDEX = "activityIndex";
var EXPERIENCE_INDEX = "experienceIndex";
var PREVIEW_TOKEN = "at_preview_token";
var PREVIEW_INDEX = "at_preview_index";
var PREVIEW_LISTED_ACTIVITIES_ONLY = "at_preview_listed_activities_only";
var PREVIEW_EVALUATE_AS_TRUE_AUDIENCE_IDS = "at_preview_evaluate_as_true_audience_ids";
var PREVIEW_EVALUATE_AS_FALSE_AUDIENCE_IDS = "at_preview_evaluate_as_false_audience_ids";
var CUSTOMER_IDS = "customerIds";
var PAYLOAD = "payload";
var AAM_PARAMETERS = "aamParameters";
var AAM_VISITOR_ID = "marketingCloudVisitorId";
var AAM_LOCATION_HINT = "dcsLocationHint";
var AAM_BLOB = "blob";
...
function parseActivityAndExperience(value) {
  var result = {};
  ...
  result[ACTIVITY_INDEX] = activityIndex;
  var experienceIndex = getIndex(pair[1]);
  if (!isNil(experienceIndex)) {
    result[EXPERIENCE_INDEX] = experienceIndex;
  }
  return result;
}
function extractActivitiesAndExperiences(values) {
  return filter(notNull, map(parseActivityAndExperience, values));
}
function extractQaMode(queryString) {
  ...
  var result = {};
  var qaMode = {};
  result[QA_MODE] = qaMode;
  qaMode[TOKEN] = token;
  var listedActivitiesOnly = query[PREVIEW_LISTED_ACTIVITIES_ONLY];
  if (isNotBlank(listedActivitiesOnly) && listedActivitiesOnly === TRUE) {
    qaMode[LISTED_ACTIVITIES_ONLY] = true;
  }
  var trueAudienceIdsValue = query[PREVIEW_EVALUATE_AS_TRUE_AUDIENCE_IDS];
  if (isNotBlank(trueAudienceIdsValue)) {
    qaMode[EVALUATE_AS_TRUE_AUDIENCE_IDS] = extractAudienceIds(trueAudienceIdsValue);
  }
  var falseAudienceIdsValue = query[PREVIEW_EVALUATE_AS_FALSE_AUDIENCE_IDS];
  if (isNotBlank(falseAudienceIdsValue)) {
    qaMode[EVALUATE_AS_FALSE_AUDIENCE_IDS] = extractAudienceIds(falseAudienceIdsValue);
  }
  var activitiesAndExperiencesValues = query[PREVIEW_INDEX];
  if (isEmpty(activitiesAndExperiencesValues)) {
    return result;
  }
  if (!isArray(activitiesAndExperiencesValues)) {
    activitiesAndExperiencesValues = [activitiesAndExperiencesValues];
  }
  var indexes = extractActivitiesAndExperiences(activitiesAndExperiencesValues);
  if (isEmpty(indexes)) {
    return result;
  }
  qaMode[PREVIEW_INDEXES] = indexes;
  return result;
}
function initQaMode(win) {
  ...
  setCookie(QA_MODE, JSON.stringify(result), { expires: expires });
}
function getQaMode() {
  var result = getCookie(QA_MODE);
  if (isBlank(result)) {
    return {};
  }
  try {
...
function createBrowser() {
  ...
  result[REFERRING_URL] = index$3.referrer;
  result[USER_AGENT] = index$2.navigator.userAgent;
  result[WEB_GL_RENDERER] = getWebGLRenderer();
  return result;
}
...
function createScreen() {
  ...
  result[ORIENTATION] = getScreenOrientation();
  result[COLOR_DEPTH] = screen.colorDepth;
...
function createContext() {
  var result = {};
  result[CHANNEL] = WEB_CHANNEL;
  result[BROWSER] = createBrowser();
  result[WINDOW] = createWindow();
  result[SCREEN] = createScreen();
  result[TIME_OFFSET] = -new Date().getTimezoneOffset();
  return result;
}
function createDeviceId(deviceId) {
  var ids = {};
  ids[DEVICE_ID] = deviceId;
  return ids;
}
function createId(visitorValues) {
  var ids = {};
  var deviceId = getDeviceId();
  if (isNotBlank(deviceId)) {
    ids[DEVICE_ID] = deviceId;
  ...
  var mid = visitorValues[MCMID];
  if (isNotBlank(mid)) {
    ids[AAM_VISITOR_ID] = mid;
  }
  var customerIds = getVisitorCustomerIdsValues();
  if (!isEmpty(customerIds)) {
    ids[CUSTOMER_IDS] = customerIds;
  }
  var result = {};
  if (!isEmpty(ids)) {
    result[ID$1] = ids;
...
function createAamParameters(visitorValues) {
  var aamParams = {};
  var locationHint = visitorValues[MCAAMLH];
  if (!isNaN(parseInt(locationHint, 10))) {
    aamParams[AAM_LOCATION_HINT] = locationHint;
  }
  var blob = visitorValues[MCAAMB];
  if (isNotBlank(blob)) {
    aamParams[AAM_BLOB] = blob;
  }
  var result = {};
  if (!isEmpty(aamParams)) {
    result[AAM_PARAMETERS] = aamParams;
  }
  return result;
}
...
function createDeliveryPayload(config, visitorValues) {
  var client = {};
  client[CLIENT] = config[CLIENT_CODE];
  var id = createId(visitorValues);
  var aamParameters = createAamParameters(visitorValues);
  var context = {};
  context[CONTEXT] = createContext();
  var qaMode = getQaMode();
  return index$1({}, client, id, aamParameters, context, qaMode);
}
function createPrefetchPayload(params, profileParams) {
  var prefetch = {};
  prefetch[PARAMETERS] = params;
  prefetch[PROFILE_PARAMETERS] = profileParams;
  var result = {};
  result[PREFETCH] = prefetch;
  return result;
}
function createNotification(params, profileParams, events) {
  var result = {};
  result[NOTIFICATION_ID] = uuid();
  result[EVENTS] = events;
  result[PARAMETERS] = params;
  result[PROFILE_PARAMETERS] = profileParams;
  return result;
}
function createNotificationsPayload(notifications) {
  var result = {};
  result[NOTIFICATIONS] = notifications;
  var qaMode = getQaMode();
  return index$1({}, result, qaMode);
}
function decoratePayloadWithDeviceId(payload) {
  var deviceId = getDeviceId();
  var result = index$1({}, payload);
  if (isBlank(deviceId)) {
    return result;
  }
  if (isNil(result[ID$1])) {
    result[ID$1] = createDeviceId(deviceId);
    return result;
  }
  result[ID$1][DEVICE_ID] = deviceId;
  return result;
}
...
function createEventWithToken(type, eventToken) {
  var result = createEvent(type);
...
function getProfileParams(parameters) {
  return reduce(function (acc, value, key) {
    if (!isProfileParam(key)) {
      return acc;
    }
...
function getServerDomain$1(client, serverDomain, overrideMboxEdgeServer) {
  if (!overrideMboxEdgeServer) {
    return serverDomain;
  }
  ...
  return serverDomain.replace(client, "" + EDGE_SERVER_PREFIX$1 + cluster);
}
function getUrl$2(scheme, serverDomain, endpoint) {
  return scheme + "//" + serverDomain + endpoint;
}
function createQueryString(client, sessionId) {
  var params = {};
  params[CLIENT] = client;
  params[SESSION_ID$1] = sessionId;
  return stringifyQueryString(params);
}
function createUrl$1(config) {
  var scheme = config[SCHEME];
  var endpoint = config[ENDPOINT];
  var client = config[CLIENT_CODE];
  var overrideMboxEdgeServer = config[OVERRIDE_MBOX_EDGE_SERVER];
  var sessionId = getSessionId();
  var serverDomain = getServerDomain$1(client, config[SERVER_DOMAIN], overrideMboxEdgeServer);
  var url = getUrl$2(scheme, serverDomain, endpoint);
  var queryString = createQueryString(client, sessionId);
...
function hasMetrics(view) {
  return !isEmpty(view[METRICS]);
}
...
function hasUngrouped(res) {
  if (!hasPrefetch(res)) {
    return false;
  }
  return !isEmpty(res[PREFETCH][UNGROUPED]);
}
function isValidOption(option) {
  return isNotBlank(option[EVENT_TOKEN]) && hasActions(option);
}
function isValidMetric(metric) {
  return isNotBlank(metric[TYPE]) && isNotBlank(metric[SELECTOR]) && isNotBlank(metric[EVENT_TOKEN]);
}
function isValidView(view) {
  return hasOptions(view) || hasMetrics(view);
}
...
function handleError$5(response) {
  var error = response[ERROR];
  if (isNotBlank(error)) {
    var exception = {};
    exception[STATUS] = ERROR;
...
function processOptions$1(viewName, options) {
  ...
  var validOptions = filter(isValidOption, options);
...
function processMetrics(metrics) {
  ...
  return filter(isValidMetric, metrics);
}
function preprocessViews(views) {
  var validViews = filter(isValidView, views);
  return map(function (view) {
    var options = processOptions$1(view[NAME], view[OPTIONS]);
    var metrics = processMetrics(view[METRICS]);
    if (!isEmpty(options)) {
...
function persistView(view, details) {
  view[PAYLOAD] = details[PAYLOAD];
  view[PARAMETERS] = details[PARAMETERS];
  view[PROFILE_PARAMETERS] = details[PROFILE_PARAMETERS];
  saveView(view);
}
function persistViews(views, details) {
  var persist = function persist(view) {
    return persistView(view, details);
  };
  forEach(persist, views);
}
function createUngroupedView(response) {
  var result = {};
  var ungrouped = response[PREFETCH][UNGROUPED];
  var metrics = ungrouped[METRICS];
  result[NAME] = UNGROUPED;
  result[OPTIONS] = ungrouped[OPTIONS];
  if (!isNil(metrics)) {
    result[METRICS] = metrics;
  }
  return result;
}
function handleViews(details) {
  var response = details[RESPONSE];
  if (!hasViews(response) && !hasUngrouped(response)) {
    return [];
  }
  var views = [];
  if (hasViews(response)) {
    views.push.apply(views, response[PREFETCH][VIEWS]);
  }
  if (hasUngrouped(response)) {
    views.push(createUngroupedView(response));
  }
  persistViews(preprocessViews(views), details);
  return views;
}
...
function createHeaders$2() {
  var result = {};
  result[CONTENT_TYPE$1] = [TEXT_PLAIN];
  return result;
}
function createXhrRequest(config, data) {
  var request = {};
  request[METHOD] = POST;
  request[URL] = createUrl$1(config);
  request[HEADERS] = createHeaders$2();
  request[DATA$1] = JSON.stringify(data);
...
function createDeliveryRequest(config, parameters, arr) {
  var visitorValues = arr[0];
  var dataProvidersParams = arr[1];
  var allParams = index$1({}, dataProvidersParams, parameters);
  var result = {};
  result[PAYLOAD] = createDeliveryPayload(config, visitorValues);
  result[PARAMETERS] = getParams(allParams);
  result[PROFILE_PARAMETERS] = getProfileParams(allParams);
  return result;
}
function createPrefetchRequest(config, details) {
  var payload = details[PAYLOAD];
  var params = details[PARAMETERS];
  var profileParams = details[PROFILE_PARAMETERS];
  var prefetch = createPrefetchPayload(params, profileParams);
  var data = index$1({}, payload, prefetch);
  details[REQUEST] = createXhrRequest(config, data);
  return details;
}
...
function createAsyncDeliveryRequest(config, parameters) {
  return getAsyncThirdPartyData().then(function (arr) {
    return createDeliveryRequest(config, parameters, arr);
  });
}
function transformAjaxResponse(details) {
  var handleDeviceId = function handleDeviceId(data) {
    saveDeviceId$1(setDeviceId, data[RESPONSE] || {});
    return data;
  };
  var handleEdgeCluster = function handleEdgeCluster(data) {
    saveEdgeCluster$1(setEdgeCluster, data[RESPONSE] || {});
    return data;
  };
  return flow([handleDeviceId, handleEdgeCluster, handleError$5, handleViews])(details);
}
function processAjaxResponse$1(request, response) {
  ...
  try {
    ...
    result[PROFILE_PARAMETERS] = request[PROFILE_PARAMETERS];
    return result;
  } catch (e) {
    return createError$2(e.message || JSON_ERROR$1);
  }
}
function executeXhr(request) {
  var req = request[REQUEST];
  var process = function process(response) {
    return processAjaxResponse$1(request, response);
  };
  return xhr(req).then(process);
}
function executePrefetch(config, params) {
  return createAsyncDeliveryRequest(config, params).then(function (details) {
    return createPrefetchRequest(config, details);
  }).then(executeXhr);
}
function executeNotifications(config, payload, notifications) {
  var notificationsPayload = createNotificationsPayload(notifications);
  var data = index$1({}, payload, notificationsPayload);
  var request = createXhrRequest(config, data);
  return xhr(request);
}

var RENDER_OPTION = "[option-render]";
var isRedirectAction$1 = function isRedirectAction(action) {
  return !isNil(action[URL]);
};
function createDetails(viewName, eventToken) {
  var result = {};
  result[VIEW_NAME] = viewName;
  result[EVENT_TOKEN] = eventToken;
  return result;
}
function createEventPayload(mbox, selectors) {
  var result = {};
  result[MBOX] = mbox;
  result[MESSAGE] = MISSING_SELECTORS;
...
function handleError$6(mbox, actions) {
  var getSelector = function getSelector(action) {
    return action[SELECTOR];
  };
  var isSelector = function isSelector(selector) {
  ...
  var selectors = filter(isSelector, map(getSelector, actions));
  var payload = createEventPayload(mbox, selectors);
  var trace = createTrace$2(payload);
  logWarn(MISSING_SELECTORS, actions);
  addTrace(CLIENT_TRACES, trace);
  notifyRenderingFailed(payload);
  return {};
}
function handleSuccess$3(mbox, viewName, eventToken) {
  var payload = {};
  payload[MBOX] = mbox;
  logDebug(ACTIONS_RENDERED);
  notifyRenderingSucceeded(payload);
  return createDetails(viewName, eventToken);
}
function cloneActions$1(actions, page) {
  return map(function (e) {
    var result = index$1({}, e);
    result[PAGE] = page;
    return result;
  }, actions);
}
function executeRendering(handleOptionHidden, option, page) {
  if (!isDeliveryEnabled()) {
    logWarn(DELIVERY_DISABLED);
    return resolve({});
  }
  ...
  var actions = cloneActions$1(option[ACTIONS], page);
  var eventToken = option[EVENT_TOKEN];
  var payload = {};
  var mbox = config[GLOBAL_MBOX_NAME];
  payload[MBOX] = mbox;
  if (isEmpty(actions)) {
    logDebug(RENDER_OPTION, NO_ACTIONS);
    handleOptionHidden();
    notifyRenderingNoOffers(payload);
    return resolve({});
  }
  ...
  if (!isNil(redirectAction)) {
    payload[URL] = redirectAction[URL];
    logDebug(RENDER_OPTION, REDIRECT_ACTION);
    notifyRenderingRedirect(payload);
    executeRedirect(redirectAction);
    return resolve(createDetails(viewName, eventToken));
  }
  notifyRenderingStart(payload);
  return executeRender(handleOptionHidden, noop, actions).then(function () {
    return handleSuccess$3(mbox, viewName, eventToken);
  })['catch'](function (arr) {
    return handleError$6(mbox, arr);
  });
}
...
function sendMetricNotifications(config, viewName, view, eventToken) {
  var currentView = getCurrentView();
  if (viewName !== UNGROUPED && currentView[VIEW_NAME] !== viewName) {
    return;
  }
  var payload = view[PAYLOAD];
  var params = view[PARAMETERS];
  var profileParams = view[PROFILE_PARAMETERS];
  var events = [createEventWithToken(CLICK_EVENT, eventToken)];
  var notifications = [createNotification(params, profileParams, events)];
  executeNotifications(config, payload, notifications);
}
function handleMetricEvent(config, metric, viewName, view) {
  var type = metric[TYPE];
  var selector = metric[SELECTOR];
  var eventToken = metric[EVENT_TOKEN];
  var css = "mt-" + hash(viewName);
  var func = function func() {
    sendMetricNotifications(config, viewName, view, eventToken);
  };
  if (!hasClass(css, selector)) {
    addClass(css, selector);
    addClass(CLICK_TRACKING_CSS_CLASS, selector);
    listen(type, func, selector);
  }
}
function applyMetric(taskId, config, metric, viewName, view, resolve$$1, reject$$1) {
  var timeoutEvent = TIMEOUT_EVENT + "-" + taskId;
  var renderCompleteEvent = RENDER_COMPLETE_EVENT + "-" + taskId;
  subscribe(TICK_EVENT, function () {
    ...
    handleMetricEvent(config, metric, viewName, view);
    publish(renderCompleteEvent);
  });
  subscribeOnce(renderCompleteEvent, function () {
    stopTicker$2();
    unsubscribe$1(TICK_EVENT);
    unsubscribe$1(timeoutEvent);
    resolve$$1(metric);
  });
  subscribeOnce(timeoutEvent, function () {
  ...
  startTicker$2(function () {
    return publish(TICK_EVENT);
  });
}
function executeMetric(config, metric, viewName, view) {
  var selectorsPollingTimeout = config[SELECTORS_POLLING_TIMEOUT];
  var taskId = uuid();
  startTimer$1(taskId, selectorsPollingTimeout);
  return create(function (resolve$$1, reject$$1) {
    applyMetric(taskId, config, metric, viewName, view, resolve$$1, reject$$1);
  });
}
...
var RENDERED_VIEW = "Rendered view";
...
var METRICS_EXECUTION = "Execute metrics";
var VIEW_NOT_FOUND = "View not found";
var METRIC_SUCCESS = "Metric executed successfully";
var METRIC_ERROR = "Metric executed with errors";
var NO_DISPLAY_NOTIFICATIONS = "No display notifications";
var NO_NOTIFICATIONS = "No notifications";
var TASK_QUEUE = queue();
function handleOptionHidden() {
  publish(OPTION_HIDDEN_EVENT);
}
function handleHidingSnippetRemoval$1(evt) {
  subscribe(evt, function () {
    removeHidingSnippetStyle();
    unsubscribe$1(evt);
  });
}
function createNotifications(params, profileParams, arr) {
  return map(function (e) {
    return createNotification(params, profileParams, [createEventWithToken(DISPLAY_EVENT, e[EVENT_TOKEN])]);
  }, arr);
}
function creatDisplayNotification(params, profileParams) {
  var events = [createEvent(DISPLAY_EVENT)];
  return createNotification(params, profileParams, events);
}
function createRenderedDisplayNotification(config, viewParams, viewProfileParams, viewName) {
  if (viewName === UNGROUPED) {
    return null;
  }
  var mboxName = config[GLOBAL_MBOX_NAME];
  var dataProvidersParams = getSyncDataProvidersParameters();
  var targetPageParams = getTargetPageParamsParameters(mboxName);
  var allParams = index$1({}, viewParams, viewProfileParams, dataProvidersParams, targetPageParams);
  var params = getParams(allParams);
  var profileParams = getProfileParams(allParams);
  return creatDisplayNotification(params, profileParams);
}
function sendViewNotifications(config, view, viewName, arr) {
  var payload = decoratePayloadWithDeviceId(view[PAYLOAD]);
  var params = view[PARAMETERS];
  var profileParams = view[PROFILE_PARAMETERS];
  var notifications = createNotifications(params, profileParams, arr);
  var notification = createRenderedDisplayNotification(config, params, profileParams, viewName);
  if (!isNil(notification)) {
    notifications.push(notification);
  }
  executeNotifications(config, payload, notifications).then(logDebug)['catch'](logWarn);
}
function sendViewDisplayNotification(config) {
  var mboxName = config[GLOBAL_MBOX_NAME];
  var parameters = index$1({}, getTargetPageParamsParameters(mboxName));
  createAsyncDeliveryRequest(config, parameters).then(function (details) {
    var payload = details[PAYLOAD];
    var params = details[PARAMETERS];
    var profileParams = details[PROFILE_PARAMETERS];
    var notifications = [creatDisplayNotification(params, profileParams)];
    return executeNotifications(config, payload, notifications);
  }).then(logDebug)['catch'](logWarn);
}
function handleRenderingSuccess$1(config, view, viewName, page, arr) {
  if (!page) {
    logDebug(NO_NOTIFICATIONS, viewName);
    return;
  }
  logDebug(RENDERED_VIEW, viewName);
  var notEmpty = function notEmpty(e) {
    return !isEmpty(e);
  };
  var details = filter(notEmpty, arr);
  if (isEmpty(details)) {
    return;
  }
  sendViewNotifications(config, view, viewName, details);
}
function handleRenderingError$1(viewName, err) {
  logWarn(RENDERING_VIEW_FAILED, viewName, err);
  publish(DELIVERY_FAILED_EVENT);
}
function handleRendering(config, view, viewName, page, done) {
  logDebug(RENDERING_VIEW, viewName);
  var success = function success(arr) {
    done();
    handleRenderingSuccess$1(config, view, viewName, page, arr);
  };
  var error = function error(err) {
    done();
    handleRenderingError$1(viewName, err);
  };
  var render = function render(opt) {
    return executeRendering(handleOptionHidden, opt, page);
  };
  var promises = map(render, view[OPTIONS]);
  all(promises).then(success)['catch'](error);
}
function handleMetrics(config, view, viewName, done) {
  if (isEmpty(view[METRICS])) {
    done();
    return;
  }
  logDebug(METRICS_EXECUTION, viewName);
  var success = function success(metric) {
    done();
    logDebug(METRIC_SUCCESS, metric, viewName);
  };
  var error = function error(err) {
    done();
    logDebug(METRIC_ERROR, err, viewName);
  };
  var execute = function execute(m) {
    return executeMetric(config, m, viewName, view);
  };
  var promises = map(execute, view[METRICS]);
  all(promises).then(success)['catch'](error);
}
function renderView(config, options) {
  if (!hasSavedViews()) {
  ...
  var viewName = options[VIEW_NAME];
  if (isBlank(viewName)) {
    logDebug(VIEW_NAME_ERROR, viewName);
    publish(OPTION_HIDDEN_EVENT);
    return;
  }
  ...
  if (isObject(view)) {
    TASK_QUEUE.push(function (done) {
      handleRendering(config, view, viewName, page, done);
    });
    TASK_QUEUE.push(function (done) {
      handleMetrics(config, view, viewName, done);
    });
    return;
  }
  if (viewName !== UNGROUPED) {
    logDebug(VIEW_NOT_FOUND, viewName);
    publish(OPTION_HIDDEN_EVENT);
    if (!page) {
      logDebug(NO_DISPLAY_NOTIFICATIONS, viewName);
      return;
    }
    sendViewDisplayNotification(config);
  }
}
function handleSuccess$4(config, views) {
  if (isEmpty(views)) {
  ...
  unsubscribe$1(DELIVERY_FAILED_EVENT);
  var options = {};
  options[VIEW_NAME] = UNGROUPED;
  options[PAGE] = true;
  renderView(config, options);
  renderView(config, getCurrentView());
}
function handleError$7(error) {
  logWarn(VIEW_DELIVERY_ERROR, error);
  publish(DELIVERY_FAILED_EVENT);
}
function handleViewStart$1(config, options) {
  setCurrentView(options);
  renderView(config, getCurrentView());
}
function subscribeToViewStart(config) {
  subscribe(VIEW_START, function (options) {
    return handleViewStart$1(config, options);
  });
}
function initDelivery() {
  ...
  subscribeToViewStart(config);
  initQaMode(index$2);
  var mboxName = config[GLOBAL_MBOX_NAME];
  var params = index$1({}, getTargetPageParamsParameters(mboxName));
  handleHidingSnippetRemoval$1(DELIVERY_FAILED_EVENT);
  handleHidingSnippetRemoval$1(OPTION_HIDDEN_EVENT);
  injectHidingSnippetStyle();
  executePrefetch(config, params).then(transformAjaxResponse).then(function (views) {
    return handleSuccess$4(config, views);
  })['catch'](handleError$7);
}
function getOptions(viewName, opts) {
  var result = {};
  result[VIEW_NAME] = viewName;
  result[PAGE] = true;
  if (!isEmpty(opts)) {
...
function triggerView(value, opts) {
  if (!isString(value) || isBlank(value)) {
    logWarn(VIEW_NAME_ERROR, value);
    return;
  }
  var viewName = value.toLowerCase();
  var options = getOptions(viewName, opts);
